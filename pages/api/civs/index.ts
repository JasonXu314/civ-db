	...
	try {
		...
		if (queryCiv && queryCiv !== '') {
			if (Array.isArray(queryCiv)) {
				res.status(200).json(
					(await mongoClient.db('civ-db').collection<WithObjectId<Civ>>('civs').find().toArray()).filter(
			...
			res.status(200).json(await mongoClient.db('civ-db').collection<WithObjectId<Civ>>('civs').find().toArray());
		}
	} catch (err) {
		console.log(err);
